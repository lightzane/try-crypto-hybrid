...
export async function decrypt(cipherText: ArrayBuffer, privateKey: CryptoKey) {
...
export async function exportPublicKeyAsPem(publicKey: CryptoKey) {
...
export async function importPublicKeyFromPem(pem: string) {
  ...
  const spki = Uint8Array.from(window.atob(base64), (c) => c.charCodeAt(0))
...
export async function importPrivateKeyFromPem(pem: string) {
  const base64 = pem
    ...
    .replace(/\s/g, '') // Remove all whitespace
  const pkcs8 = Uint8Array.from(window.atob(base64), (c) => c.charCodeAt(0))
  return webcrypto.subtle.importKey(
    'pkcs8', // format
    pkcs8, // key data
    RSA_OAEP_ALGO,
